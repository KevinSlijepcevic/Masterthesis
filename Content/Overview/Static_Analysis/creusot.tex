...
Creusot is a \textit{deductive} verification tool for Rust whose development has started in October of 2020\footnote{According to the tool's corresponding GitHub log \cite{xavier_denis_initial_2020}}. The verifying process is semi-automatic: Creusot automatically generates theories from a given Rust code extended with Creusot annotations; these theories have to be imported into Why3 and evaluated there \cite{riesco_creusot_2022}. Creusot extends the idea of \textit{prophecies}, which have been first introduced to Rust by Matsushita et al., in a project called \textit{RustHorn} \cite{matsushita_rusthorn_2021, riesco_creusot_2022}. Creusot uses a language called \textit{Pearlite} for designing its specifications, which represents a pure subset of the Rust programming language extended with logical expressions \cite{xavier_denis_xldeniscreusot_2023,riesco_creusot_2022}.

\subsubsection{Usability}

Like Prusti (refer to Chapter \ref{cap:prusti_usability}), Creusot integrates itself into the Rust compiler and uses the generated MIR for its theory generation \cite{riesco_creusot_2022}. It can also be used via cargo \cite{denis_creusot_2021} The automatically generated theories produces from Creusot have to be manually imported into Why3, although there exists a convenient wrapper for that purpose \cite{riesco_creusot_2022, xavier_denis_xldeniscreusot_2023}.
...
Creusot relies on Rust's borrow checker, it does not create core proofs like Prusti for example (see Chapter \ref{cap:prusti_language_verification}) \cite{riesco_creusot_2022}. The authors believe that omitting this significantly reduces verification time effort, compared to Prusti \cite{riesco_creusot_2022}. 
...
Creusot supports access to models of data structures \cite{denis_creusot_2021}. For example, the Vec<T> data structure can be viewed as a mathematical sequence of values (Seq<T>) \cite{denis_creusot_2021}. It is possible to design models for custom data structures, implementing the \lstinline{Model} trait \cite{denis_creusot_2021,xavier_denis_xldeniscreusot_2023}.

Creusot leverages prophecies into modelling mutable borrows. It enables the tool to access the final value of a mutable borrow. This functionality can be compared to Prusti's pledges (see Chapter \ref{cap:prusti_language_verification} \cite{denis_creusot_2021,riesco_creusot_2022}). The distinction is based on the consideration of time: Prusti's pledges are checked at the end of the mutable borrows lifetime, while Creusot specifies the future values of a mutable borrow \cite{denis_creusot_2021}.

Creusot supports ghost types, which only exist during verification, which can be used to store the state of a value before reaching a loop for example \cite{riesco_creusot_2022}.

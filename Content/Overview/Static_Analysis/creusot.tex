\subsection{Creusot}

Creusot is a \textit{deductive} verification tool for Rust whose development has started in 2020. The verifying process is semi-automatic: Creusot automatically generates theories from a given Rust code extended with Creusot annotations; these theories have to be imported into Why3 and evaluated there \cite{riesco_creusot_2022}. Creusot extends the idea of \textit{prophecies}, which have been first introduced to Rust by Matsushita et al. \cite{matsushita_rusthorn_2021, riesco_creusot_2022}. Creusot uses a language called \textit{Pearlite} for designing its specifications, which represents a pure subset of the Rust programming language extended with keywords and contains additional mathematical data types like Seq<T> and Int \cite{xavier_denis_xldeniscreusot_2023,riesco_creusot_2022}.

\subsubsection{Usability}

Like Prusti (refer to Chapter \ref{cap:prusti_usability}), Creusot integrates itself into the Rust compiler and uses the generated MIR for its theory generation \cite{riesco_creusot_2022}. The automatically generated theories produces from Creusot have to be manually imported into Why3, although there exists a convenient wrapper for that purpose \cite{riesco_creusot_2022, xavier_denis_xldeniscreusot_2023}.
From a developer standpoint, that results in additional effort and time required to import the generated theories into Why3 and evaluate them there. It also requires domain knowledge for formal verification and for using the Why3 ecosystem. 
From a formal verification expert standpoint, it opens a broad field of customization for specific needs e.g. setting a different timeout for the SMT solver for one particular theory or using different proof tactics.
It is planed to improve this part of the process though, in form of a Visual Studio Code plugin which acts as a frontend for Why3 \cite{xavier_denis_xldeniswhycode_2023}.

The annotations written in Pearlite are similar to those in Prusti, using the same Rust specific attributes style \cite{rust_foundation_attributes_nodate}, as can be seen in \autoref{lst:creusot_pre_postcondition}. Note that numbers in the specifications have to be equipped with the desired datatype for arithmetic operations, as Creusot automatically infers them as a Pearlite specific integer. Also note the practical appliance of the prophecy logic, which is annotated by the \textsuperscript{$\wedge$} operator in the postcondition, which implies the final value of \textit{val} and the * operator, which implies the current value of \textit{val} \cite{riesco_creusot_2022}.

It is also possible to call functions inside of the specifications, marked with the \lstinline{#[predicate]} or \lstinline{#[logic]} annotation. Predicates are typically functions expecting Pearlite data types, preventing them from being called outside of verification purposes \cite{riesco_creusot_2022}. However, it is also possible to design helper predicate functions with original Rust data types. 

\begin{lstlisting}[language=Rust, caption=Example Rust code of a function with a pre- and postcondition for Creusot, label={lst:creusot_pre_postcondition}]
#[requires(*val <= u32::MAX - 1u32)]
#[ensures(^val == *val + 1u32)]
fn incr(val: &mut u32) {
    *val += 1;
}
\end{lstlisting}

\subsubsection{Technology}
Creusot translates Rust Code with Pearlite annotations into the WhyML language, which is designed to be executed by Why3 \cite{riesco_creusot_2022}. To represent the unstructured nature of the generated MIR, a WhyML language extension called \textit{MLCFG} is used by Creusot \cite{riesco_creusot_2022,the_why3_development_team_7_2023}. The tool therefore uses first-order logic for its reasoning \cite{riesco_creusot_2022}.

Creusot relies on Rust's borrow checker, it does not create core proofs like Prusti for example (see Chapter \ref{cap:prusti_language_verification}) \cite{riesco_creusot_2022}. The authors believe that omitting this significantly reduces verification time effort, compared to Prusti \cite{riesco_creusot_2022}. 

Due to the usage of Why3 for proofing the generated theories, the tool relies on a well-known and well-tested verification platform \cite{riesco_creusot_2022}. Why3 supports a wide variety of automatic provers, like CVC3, CVC4, Z3, Alt-Ergo and interactive provers like Coq or Isabelle \cite{why3_why3_2023}. 

\subsubsection{Feedback}
Due to Creusot's integration into the Rust compiler, error messages regarding the Pearlite syntax are presented like regular Rust errors. Because Creusot does not perform actual reasoning by itself though, logic errors in the specification are not reported through the Rust compiler. Instead, the user has to interpret the resulting Why3 behaviour, which adds a layer of abstraction over the verification process and can make the user experience challenging, especially for beginners. However, if the user possess advanced experience in the operation of the Why3 verification framework, this process can provide more detailed results.

\subsubsection{Language Support and Verification Conditions}
Creusot can only process safe Rust code \cite{riesco_creusot_2022}. Unsafe Rust code can be encapsulated by a safe abstraction layer, which can be marked as trusted \cite{riesco_creusot_2022}. Creusot makes use of this technique by shipping a, with annotations retrofitted, subset of Rust's standard library with the tool \cite{denis_creusot_2021}.

Creusot supports a wide variety of the Rust syntax like traits, structs, enums, borrowing, loops and recursive function calls \cite{denis_creusot_2021}.

Creusot supports access to models of data structures \cite{denis_creusot_2021}. For example, the Vec<T> data structure can be viewed as a mathematical sequence of values (Seq<T>) \cite{denis_creusot_2021}. It is possible to design models for custom data structures, implementing the \lstinline{Model} trait \cite{denis_creusot_2021}.

Creusot leverages prophecies into modelling mutable borrows. It enables the tool to access the final value of a mutable borrow. This functionality can be compared to Prusti's pledges (see Chapter \ref{cap:prusti_language_verification} \cite{denis_creusot_2021,riesco_creusot_2022}). The distinction is based on the consideration of time: Prusti's pledges are checked at the end of the mutable borrows lifetime, while Creusot specifies the future values of a mutable borrow \cite{denis_creusot_2021}.

Creusot supports ghost types, which only exist during verification, which can be used to store the state of a value before reaching a loop for example \cite{riesco_creusot_2022}.

Additonally, Creusot supports
\begin{itemize}
    \item Pre- and postconditions \cite{riesco_creusot_2022}
    \item Predicates, quantifiers, loop invariants and variants, implications \cite{riesco_creusot_2022}
    \item User specific lemmas \cite{riesco_creusot_2022}
    \item Logical equality \cite{riesco_creusot_2022}
    \item Inline assertions
\end{itemize}

\subsubsection{Development}
\subsubsection{Activity}
\subsubsection{Scientific activity}
\subsubsection{Documentation}
% Testsuite mit praktischen Beispielen vorhanden, 
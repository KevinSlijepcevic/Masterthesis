...
The developer already has plans to improve this part of the process though, in form of a Visual Studio Code plugin which acts as a frontend for Why3 \cite{xavier_denis_xldeniswhycode_2023}.

The annotations written in Pearlite are quite similar to those in Prusti as well, using the same Rust specific attributes style \cite{rust_foundation_attributes_nodate}. As can be seen, numbers have to converted into the desired datatype, as the default Pearlite number datatype is \textit{Int}, which is only present in the Pearlite context

...
There seems to be a problem with the usage of the contracts crate in combination with MIRAI. If the postcondition for the increase function in \autoref{lst:mirai_contracts_annotations_pre_postcondition} states that the mutable reference \lstinline{val} has to be the same values as prior to execution, MIRAI does not issue a verification error. MIRAI does issue an potential overflow error, if the postcondition requires a greater result than the function actually results in, e.g. \lstinline{#[ensures(*val == old(*val) + 2)]}. This problem does not occur if the mirai-annotations crate is used for designing the postcondition.

Additionally, due to the integration in the cargo build process, MIRAI sometimes does not run it's diagnostics as expected, if the Rust program has previously been compiled. Clearing build artifacts with \lstinline{cargo clean} command fixes this problem.
...
The supplementary documentation regarding MIRAI does not state any lack of support for certain Rust features. Furthermore, it does not state whether it does or does not support unsafe Rust code. However, while testing MIRAI, I was able to verify postconditions where prior unsafe operations have been executed. This seems to conclude that MIRAI can handle and prove unsafe Rust code.  

Using invariants is possible via the contracts crate, the mirai-annotations crate has no support for this feature \cite{facebookexperimental_miraidocumentationoverviewmd_2021}. Invariants can be only applied to struct fields \cite{facebookexperimental_miraidocumentationoverviewmd_2021}.
...
MIRAI is being actively supported with over 1,100 commits in the main branch since 2018 \cite{facebookexperimental_facebookexperimentalmirai_2023}. The latest release at the time of writing, version 1.1.8, has been released in May 2023 \cite{facebookexperimental_release_2023} and is considered stable \cite{facebookexperimental_miraidocumentationoverviewmd_2021}. There are subtle additions planed for MIRAI like providing a counter example if runtime error freedom cannot be proven or improved support for parameterized tests \cite{facebookexperimental_miraidocumentationoverviewmd_2021}. Therefore, MIRAI can be considered nearly feature complete.

Furthermore, MIRAI has been used to statically verify the the Diem project \cite{diem_diemdiem_2023}.
...
MIRAI provides an overview \cite{facebookexperimental_miraidocumentationoverviewmd_2021} for its capabilities but does not offer a dedicated user guide. It does offer a guide and examples on how to use the tag analysis feature in MIRAI \cite{facebookexperimental_miraidocumentationtaganalysismd_2021}. It also offers a manual for the graph generator feature \cite{facebookexperimental_miraidocumentationcallgraphmd_2022}. Furthermore, both the contract \cite{karroffel_contracts_2023} and mirai-annotations \cite{herman_venter_mirai-annotations_2023} crate offer a user manual.

MIRAI also provides six code examples, on how to use its verification and tag analysis features \cite{herman_venter_mirai-annotations_2023}.
Additionally, the Diem project \cite{diem_diemdiem_2023} offers practical examples for the appliance of MIRAI in combination with the mirai-annotations crate.

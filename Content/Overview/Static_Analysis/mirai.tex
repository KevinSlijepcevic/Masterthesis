...
\begin{lstlisting}[float, language=Rust, caption=Example Rust code of an increase function with a pre- and postcondition from the \textit{contracts} crate for MIRAI, label={lst:mirai_contracts_annotations_pre_postcondition}]
...
\begin{lstlisting}[float, language=Rust, caption=Example Rust code of an increase function with a pre- and postcondition from the \textit{mirai-annotations} crate for MIRAI, label={lst:mirai_mirai_annotations_pre_postcondition}]
fn incr(val: &mut u32) {
...
%Invariants teilweise ja, quantifiers nein aber kein problem, tag analysis/taint analysis!, diagnostics für library code, call graph generator, foreign functions need model (oder sind unreachable bzw. runtime errors?) 
